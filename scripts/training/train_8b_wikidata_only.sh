...
# Training script for Llama 3.1 8B on Wikidata-only dataset
...
LOG_FILE="/home/logs/knowledge_comparison/llama31_8b_wikidata_only_training.log"
mkdir -p /home/logs/knowledge_comparison

echo "Starting Llama 3.1 8B Wikidata-only training..." | tee "$LOG_FILE"
echo "GPU: $CUDA_VISIBLE_DEVICES" | tee -a "$LOG_FILE"
echo "Config: /home/configs/knowledge_comparison/8b_wikidata_only.yaml" | tee -a "$LOG_FILE"
...
llamafactory-cli train /home/configs/knowledge_comparison/8b_wikidata_only.yaml 2>&1 | tee -a "$LOG_FILE"
