...
# Training script for Llama 3.2 3B on Wikipedia-only dataset
...
LOG_FILE="/home/logs/knowledge_comparison/llama32_3b_wikipedia_only_training.log"
mkdir -p /home/logs/knowledge_comparison

echo "Starting Llama 3.2 3B Wikipedia-only training..." | tee "$LOG_FILE"
echo "GPU: $CUDA_VISIBLE_DEVICES" | tee -a "$LOG_FILE"
echo "Config: /home/configs/knowledge_comparison/3b_wikipedia_only.yaml" | tee -a "$LOG_FILE"
...
llamafactory-cli train /home/configs/knowledge_comparison/3b_wikipedia_only.yaml 2>&1 | tee -a "$LOG_FILE"
