...
# Training script for Llama 3.1 8B on Wikipedia-only dataset
...
LOG_FILE="/home/logs/knowledge_comparison/llama31_8b_wikipedia_only_training.log"
mkdir -p /home/logs/knowledge_comparison

echo "Starting Llama 3.1 8B Wikipedia-only training..." | tee "$LOG_FILE"
echo "GPU: $CUDA_VISIBLE_DEVICES" | tee -a "$LOG_FILE"
echo "Config: /home/configs/knowledge_comparison/8b_wikipedia_only.yaml" | tee -a "$LOG_FILE"
...
llamafactory-cli train /home/configs/knowledge_comparison/8b_wikipedia_only.yaml 2>&1 | tee -a "$LOG_FILE"
