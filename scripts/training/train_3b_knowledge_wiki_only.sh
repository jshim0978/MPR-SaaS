...
# Training Script: Llama 3.2 3B - Wikidata + Wikipedia Only (NO KILT WOW)
...
mkdir -p "$LOG_DIR"
LOG_FILE="$LOG_DIR/3b_training.log"

echo "╔═══════════════════════════════════════════════════════════════════════════╗" | tee "$LOG_FILE"
echo "║   Training Llama 3.2 3B - Wikidata + Wikipedia Only (NO KILT WOW)       ║" | tee -a "$LOG_FILE"
echo "╚═══════════════════════════════════════════════════════════════════════════╝" | tee -a "$LOG_FILE"
echo "" | tee -a "$LOG_FILE"
echo "Started at: $(date)" | tee -a "$LOG_FILE"
echo "GPU: cuda:0" | tee -a "$LOG_FILE"
echo "Dataset: Wikidata + Wikipedia (24,982 samples)" | tee -a "$LOG_FILE"
echo "Output: /home/models/llama32_3b_knowledge_wiki_only_lora" | tee -a "$LOG_FILE"
echo "" | tee -a "$LOG_FILE"

cd /home/LLaMA-Factory || exit

llamafactory-cli train /home/configs/knowledge/3b_knowledge_wikidata_wikipedia_only.yaml >> "$LOG_FILE" 2>&1
