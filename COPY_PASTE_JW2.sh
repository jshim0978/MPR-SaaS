#!/bin/bash
# COMPLETE JW2 SETUP - Copy and paste this entire script
# Includes: SSH key setup, Git clone, HuggingFace access, model transfer

cat << 'EOF'
╔═══════════════════════════════════════════════════════════════════════════╗
║                  JW2: CLEANER WORKER COMPLETE SETUP                       ║
╚═══════════════════════════════════════════════════════════════════════════╝
EOF
...
if [ ! -f ~/.ssh/id_ed25519 ]; then
    ...
    ssh-keygen -t ed25519 -f ~/.ssh/id_ed25519 -N "" -C "jw2-cleaner"
...
echo ""
echo "═══════════════════════════════════════════════════════════════════════════"
echo "STEP 4: Transfer Grammar models from sbs29"
echo "═══════════════════════════════════════════════════════════════════════════"

mkdir -p /home/models

echo "Transferring 3B Grammar model (this may take 10-15 minutes)..."
rsync -avz --progress ${SBS29_USER}@${SBS29_IP}:/home/models/llama32_3b_grammar_lora/ \
    /home/models/llama32_3b_grammar_lora/

echo ""
echo "Transferring 8B Grammar model (this may take 15-20 minutes)..."
rsync -avz --progress ${SBS29_USER}@${SBS29_IP}:/home/models/llama31_8b_grammar_lora/ \
    /home/models/llama31_8b_grammar_lora/

echo "✅ Models transferred"
...
try:
    ...
    tokenizer = AutoTokenizer.from_pretrained(
        ...
        token=True  # Will use HF_TOKEN env var or cached login
    )
...
📋 Status:
  ...
  ✓ Grammar models transferred (3B + 8B)
...
echo ""
echo "✅ All done! JW2 is ready to start service."
