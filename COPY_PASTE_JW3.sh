#!/bin/bash
# COMPLETE JW3 SETUP - Copy and paste this entire script
# Includes: SSH key setup, Git clone, HuggingFace access, model transfer

cat << 'EOF'
╔═══════════════════════════════════════════════════════════════════════════╗
║                JW3: DESCRIBER WORKER COMPLETE SETUP                       ║
╚═══════════════════════════════════════════════════════════════════════════╝
EOF
...
if [ ! -f ~/.ssh/id_ed25519 ]; then
    ...
    ssh-keygen -t ed25519 -f ~/.ssh/id_ed25519 -N "" -C "jw3-describer"
...
echo ""
echo "═══════════════════════════════════════════════════════════════════════════"
echo "STEP 4: Transfer Wikipedia models from sbs29"
echo "═══════════════════════════════════════════════════════════════════════════"

mkdir -p /home/models

echo "Transferring 3B Wikipedia model (this may take 10-15 minutes)..."
rsync -avz --progress ${SBS29_USER}@${SBS29_IP}:/home/models/llama32_3b_wikipedia_only_lora/ \
    /home/models/llama32_3b_wikipedia_only_lora/

echo ""
echo "Transferring 8B Wikipedia model (this may take 15-20 minutes)..."
rsync -avz --progress ${SBS29_USER}@${SBS29_IP}:/home/models/llama31_8b_wikipedia_only_lora/ \
    /home/models/llama31_8b_wikipedia_only_lora/

echo "✅ Models transferred (Quality: 8.8/10 from evaluation)"
...
print("   Model quality: 8.8/10 (from HHEM evaluation)")
PYEOF
...
📋 Status:
  ...
  ✓ Wikipedia models transferred (3B + 8B, Quality: 8.8/10)
...
echo ""
echo "✅ All done! JW3 is ready to start service."
