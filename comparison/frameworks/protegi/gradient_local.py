"""
ProTeGi (1-pass) - Local Llama Version

Textual gradients using Llama-3.2-3B or Llama-3.1-8B.
"""
...
class ProTeGi_Local(BasePromptOptimizer):
    """
    ProTeGi using local Llama models via vLLM
    """
    ...
    def __init__(self, model_size: str = "3b"):
        ...
        super().__init__(name=f"ProTeGi_Local_{model_name}", model=model_name, max_iterations=1)
...
Output ONLY the improved prompt."""
    ...
    async def refine(self, prompt: str) -> RefinementResult:
        ...
        try:
            response = await chat(messages, temperature=0.4, max_tokens=350)
...
if __name__ == "__main__":
    ...
    async def main():
        print("="*80)
        print("ProTeGi Local Test")
        print("="*80)
        
        for model_size in ["3b", "8b"]:
            print(f"\n--- Llama-{model_size.upper()} ---")
            protegi = ProTeGi_Local(model_size=model_size)
            
            prompt = "what is the captial of frane?"
            result = await protegi.refine(prompt)
